...
\noindent State space of Mersenne Twister (standard PRNG in Statistics): \\
...
\item (New to Statistics) PRNGs based on cryptographic hash functions
\eit
\end{block}
...
\item Reservoir algorithms: Algorithm R (\cite{knuth_art_1997}), Algorithm Z (\cite{vitter_random_1985}) are related to shuffling and don't require knowing the population size a priori
\eit
\end{frame}
...
\item A common way to obtain an integer in the range $\{1, \dots, m\}$:
\bit
\item Generate a PRN $U$ on $[0, 1)$, take $\lfloor m U\rfloor + 1$
\item But $U$ is not really continuous -- it can only take $2^w$ values (where $w$ is number of bits in PRNG output)
...
\bit
\item Mimics a random generator
\bit
\item Cryptography definition: No application of the PRNG should behave differently from a ``random oracle''
\item Stats definition: We cannot reject the null hypothesis that the outputs come from a uniform distribution
\eit
\item Fast and memory efficient
\item Desirable, but not essential:
\bit 
\item Unpredictable. This is different from random - if it's deterministic, then it's predictable to some degree. Important in cryptography, less important in Statistics.
...
$R$ has a complicated distribution... use asymptotic approximation from \citet{young_two_1962}: 
...
\hyperlink{range_power}{\beamerbutton{Power}}
...
\frame{
\frametitle{Compressability tests}
 Idea: More information needed to describe structure $\iff$ less correlation between consecutive PRNs


\begin{enumerate}
\item Compressability test: measures entropy in sequences of PRNs
\item Spectral test in dimension $d$: measures the maximum distance between parallel hyperplanes that cover all points in a sequence
\end{enumerate}
...
\frame{
\frametitle{Sequential probability ratio test}
Sequential test for multinomial random variables: reduce it to a test about a Bernoulli $p$ (\cite{weiss_sequential_1962})
\bit
...
\bit
\item Mersenne Twister (MT) is a ``twisted'' generalized linear feedback shift register: a complex sequence of bitwise and linear operations
\item Enormous period of $2^{19937} - 1$, a Mersenne prime
\item $k$-distributed to 32-bit accuracy for $k \leq 623$, i.e. tuples of up to length $623$ occur with equal frequency over the period
...
\begin{center}
\textbf{One solution to the pigeonhole problem:} \\   
\vspace{20pt}
...
\frametitle{Hash function PRNGs}
\bit
\item Efficient: based on fast, pre-existing hash function code. Some cryptographic hash primitives are even built into hardware (e.g. AES block cipher is on current Intel processors)
...
\frametitle{Sampling tests}
\bit
\item None of the 3  good PRNGs is best overall -- results vary by seed value
\bit
\item SHA256 PRNG should behave like ``random oracle'' and results should not vary by seed
\item \textbf{Next step:} run tests for many seeds; p-values should be uniform under the null
\eit
\pause
\item Results vary by sample size $k$
\bit
\item Sampling $k$ from $n$ is complementary to sampling $n-k$ from $n$
\item Resulting samples are not equally uniform -- a difference between theory and practice
...
\section[Other dissertation chapters]{Other dissertation chapters}
  \begin{frame}
  \frametitle{Contents}
  \tableofcontents[currentsection]
  \end{frame}
...
\frame
{
  \frametitle{Permutation tests and gender bias}
Are student evaluations of teaching (SET) a valid measure of teaching effectiveness? \\

\vspace{15pt}
In \href{https://www.scienceopen.com/document?vid=818d8ec0-5908-47d8-86b4-5dc38f04b23e}{Boring et al. (2016)}, we reanalyzed two datasets:

\bit
\item an online, randomized experiment from North Carolina State University
\item a natural experiment at Sciences Po, Paris
\eit

\vspace{15pt}

 We designed the permutation tests carefully to match the (implied) experimental design.
\vspace{10pt}
...
Statisticians should worry about software development:
\bit
\item ethics: reproducibility crisis in science
\item impact: enable people to use the methods we develop (correctly!)
\eit

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.7\textwidth]{fig/permute}
\end{center}
\end{figure}

\bit
\item Open source software practices: the GitHub model
\item Writing robust, useable code
\item Unit testing and documentation
\eit
...
\frame{
\frametitle{Another chapter}
Project ideas:

\bit
\item Permutation tests for inter-rater reliability
\item Reproducibility lexicon
\item Applications of permutation tests (linear models, paired categorical data)
\eit
}
...
\hyperlink{range_test}{\beamerbutton{back}}
