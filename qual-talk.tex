...
\bit
\item A \textbf{pseudorandom number generator} (PRNG) 
...
\item Impossibility bounds show that this introduces a nontrivial amount of bias into statistics estimated from Monte Carlo distributions \hyperlink{impossibility_bounds}{\beamerbutton{here}}
\eit
\end{frame}
...
\bit
\item Red: require $n$ PRNs
\item Blue: require $k$ PRNs
...
\todo{
\bit
\item List and cite test suites
\item Describe a few interesting/important tests
\item Argue that for statistical use, PRNGs should be able to produce samples uniformly and this ought to be a test
\item Describe chi-squared test and range test
\item Mention Spearman correlation test of permutations?
\eit
}}
...
\item Generally considered ``random'' enough for Statistics, but fails some TestU01 tests
...
\todo{
\bit
\item Compare MT/CSPRNG/LCG with different seeds
\item For the unique sample frequencies, report
\bit
\item Range statistic $p$-value
\item Chi-squared test $p$-value
\eit
\item Do this for various numbers of samples - 1e6 up to 1e8. How do $p$-values change? How does this relate to equidistribution?
\eit
}}
